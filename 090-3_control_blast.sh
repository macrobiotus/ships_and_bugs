...
if [[ "$HOSTNAME" != "Pauls-MacBook-Pro.local" ]] && [[ "$HOSTNAME" != "macmini-fastpost.staff.uod.otago.ac.nz" ]]; then
    ...
    # locally should be Nucleotide-Nucleotide BLAST 2.10.1+
    # do not run in conda env, uncomment below if necessary 
    #   export PATH=/usr/local/bin/blastn:$PATH    
    # Blast db on external drive
...
for fasta in "${inpth_seq[@]}";do
  ...
  if [ ! -f "$trpth"/Zenodo/Qiime/090-218S_controls_tab_qiime_artefacts_control/"$tgt_file" ]; then
