...
if (length(args)==1) {
  ...
  args[3] = get_path(args[1], "_model_data", ".csv")
  ...
  args[4] = get_path(args[1], "_variables_by_ecoregion_", ".pdf")
  ...
  args[5] = get_path(args[1], "_modelling_results_", ".txt")
  
  # Definition of sixth default arguments `args[6]` will define write path 
  #  for model graphic summary in `ggsave()` call. 
  #  type = "message", split = TRUE)` command,  
  args[6] = get_path(args[1], "_modelling_results_", ".pdf")
  
  # Definition of seventh default arguments `args[7]` will define image legend 
  #  in `ggsave()` call. 
  args[7] = "Biologic distance metric changes between ports, depending on ecoregion-crossing and environmental distance (in SD)."
...
library ("gridExtra") # exporting ggplots
...
write.csv(model_data, file = args[3])
...
plots[[1]] <- ggplot(model_data, aes (x=RESP_UNIFRAC)) + 
              geom_density() +
              facet_grid(~ECO_DIFF) +
              theme_bw()

plots[[2]] <- ggplot(model_data,aes (x=PRED_ENV))+ 
              geom_density()+
              facet_grid(~ECO_DIFF)+
              theme_bw()
...
plots[[3]] <-  ggplot(model_data,aes (x=PRED_TRIPS))+ 
               geom_density()+
               facet_grid(~ECO_DIFF)+
               theme_bw()

pdf(args[4], onefile = TRUE)
...
sink(file = args[4], append = TRUE, type = "message", split = TRUE)

  message("Aggregation of biological distance means per ecoregion. (Response variable)")
  aggregate(model_data$RESP_UNIFRAC~model_data$ECO_DIFF, FUN=mean)

  message("Aggregation of environmental distance means per ecoregion (Predictor variable).")
  aggregate(model_data$PRED_ENV~model_data$ECO_DIFF, FUN=mean)

  message("Aggregation of summed voyage counts means per ecoregion (Predictor variable).")
...
# Appending to output file.
sink(file = args[5], append = TRUE, type = "message", split = TRUE)
  
  message("Considered vraiables")
  print(vars_model_full)
  
  message("Random effect model summary")
  print(summary(vars_model_full))
  
  message("Intercepts for factor levels")
  print(coef(vars_model_full)) #intercept for each level
...
plots[[4]] <- plot_model(vars_model_full, show.values = TRUE, value.offset = .3,
   type = "std", 
   title = args[7])

ggsave(filename = args[6], plot = plots[[4]],
    device = default_device(filename), path = NULL,
    scale = 1, width = par("din")[1],
    height = par("din")[2], units = "cm",
    dpi = 300)
