...
done < <(find "$trpth/Zenodo/Qiime" \( -name '128_*_seq_*.qza' \) -print0)
...
for i in "${!inpth_seq[@]}"; do
  ...
  if [ "$seqtest" == "$tabtest" ]; then
    ...
    seq_file_vis_path="$directory/129_$seq_file_name""$extension"
    tab_file_vis_path="$directory/129_$tab_file_name""$extension"
    plot_file_vis_path="$directory/129_$plot_file_name"_barplot"$extension"
