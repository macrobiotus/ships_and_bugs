...
for i in "${!inpth_seq[@]}"; do
  ...
  seqtest="$(basename "${inpth_seq[$i]//_seq_/}")"
  tabtest="$(basename "${inpth_tab[$i]//_tab_/}")"
  ...
  if [ "$seqtest" == "$tabtest" ]; then
    ...
    seq_file_name="$(basename "${inpth_seq[$i]%.*}")"
    ...
    seq_file_vis_path="$directory/$seq_file_name"_105_seq_vis"$extension"
    tab_file_vis_path="$directory/$tab_file_name"_105_tab_vis"$extension"
