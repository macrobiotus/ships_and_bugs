...
#   take 5 samples without replacement from each "RID" with with equal or more then 5 samples, discard all others
keep <- fmeDNA %>% group_by(RID, add = TRUE) %>% filter(Count  >= 49000 ) %>% tally(.) %>% filter(n >= 5) %>% select(RID)
slctDNA <- fmeDNA %>% group_by(RID, add = TRUE) %>% filter(Count  >= 49000 ) %>% filter(RID %in% keep$RID) %>% sample_n(5)
